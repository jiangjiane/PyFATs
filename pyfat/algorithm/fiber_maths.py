...
def muti_bundle_registration(paths_file, pts=12):
    ...
    sub1 = fas.get_data()
    bundle_header['fasciculus_id'] = len(sub1) * [int(filter(lambda x: x.isdigit(), paths_file[0].split('/'))[0])]
    ...
    bundle_header['fasciculus_id'] += (len(bundle) - len(sub1)) * [int(filter(lambda x: x.isdigit(), paths_file[1].split('/'))[0])]
    # print bundle_header
    # print len(bundle)
    for index in range(len(paths_file))[2:]:
        ...
        bundle_header['fasciculus_id'] += (len(bundle) - lenth) * [int(filter(lambda x: x.isdigit(), paths_file[index].split('/'))[0])]
