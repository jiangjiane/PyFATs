...
for i in range(len(fa_result[:4])):
    plt.subplots()
    # plt.title(tck_fa_name[i].split('.')[0][1:])
    # plt.plot(np.linspace(0, lengths_result[i], 200), np.array(fa_result[i]).mean(axis=0), color='green',
    #          label='Left')
    # plt.plot(np.linspace(0, lengths_result[i + 4], 200), np.array(fa_result[i + 4]).mean(axis=0), color='blue',
    #          label='Right')
    ...
    plt.savefig(os.path.join(out_results_dir, 'merge_R_{}_fa.tif'.format(tck_fa_name[i].split('.')[0][1:4])), dpi=300)
