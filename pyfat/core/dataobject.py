...
class Fasciculus(object):
    ...
    def __init__(self, source, header=None, lengths_min=None, lengths_max=None,
        ...
        if isinstance(source, nibas.ArraySequence):
            ...
            self._img = nib.streamlines.tck.TckFile.load(source)
            self._header = self._img.header
            self._datas = self._img.streamlines
        ...
        # labes
        self._labels = len(self._datas)*[None]
        # dict(self._label, self._data)
        self._labels_data = zip(self._labels, self._datas)
        # lr ratio
    ...
    def get_lengths(self):
        lengths = np.array(list(length(self._datas)))
        return lengths

    def get_counts(self):
        counts = len(self._datas)
        return counts
        ...
        if 'fasciculus_id' in self._header.keys():
            ...
            for _id in str:
                try:
                    ...
                    pass
            self._header['fasciculus_id'] = np.array(int_id)
            del int_id
            gc.collect()
        else:
            pass
    ...
    def get_mean_curvature(self):
        ...
        mean_curvature = [dtm.mean_curvature(stream) for stream in self._datas]
        return mean_curvature

    def get_mean_orientation(self):
        """Mean orientation of each streamline."""
        mean_orientation = [dtm.mean_orientation(stream) for stream in self._datas]
    ...
    def get_lr_ratio(self):
        ...
        r = []
        for i in range(len(self._datas)):
            if len(self._datas[i][:, 0][self._datas[i][:, 0] <= 0]) == 0 or \
                            len(self._datas[i][:, 0][self._datas[i][:, 0] >= 0]) == 0:
                rat = 0
            else:
                rat = len(self._datas[i][:, 0][self._datas[i][:, 0] <= 0]) / \
                      len(self._datas[i][:, 0][self._datas[i][:, 0] >= 0])
            if 0 < rat < 1:
                rat = 1 / rat
            r.append(rat)
        return r
    ...
    def xmin_nodes(self, data=None):
        ...
        for i in range(len(self._datas)):
            ...
            index_term = np.argmin((abs(self._datas[i][index0[0][0]][0]),
                                    abs(self._datas[i][index0[0][0] + 1][0])))
            index = index0[0][0] + index_term
            xmin_nodes.append(self._datas[i][index])
        return xmin_nodes
    ...
    def save2tck(self, file_path):
        """Save to a tck file"""
        tractogram = nibtcg.Tractogram(streamlines=self._datas, data_per_streamline=self._data_per_streamline,
